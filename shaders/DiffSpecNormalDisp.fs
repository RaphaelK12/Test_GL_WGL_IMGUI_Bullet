...
float rand(vec2 co){
    return fract(sin(dot(co.xy ,vec2(12.9898,78.233)+sintime.z*11)) * (43758.5453+sintime.x*10));
}
...
shading lambert(in vec3 n, in vec3 v, in light l, in vec2 uv) {
	shading s;
...
shading blinPhong1(in vec3 n, in vec3 v, in light l, in vec2 uv, vec2 dx, vec2 dy) {
	shading s;
	vec3 N =  getNormalFromMap(n, texture3, fsin.uv1, fsin.position_M.xyz,  dx,  dy);
	vec3 L = normalize(l.direction);
	vec3 V = normalize(v);
...
shading blinPhong2(in vec3 n, in vec3 v, in light l, in vec2 uv, in vec2 dx, in vec2 dy) {
	shading s;
	vec3 N = normalize(n);
	// vec3 n1 = unpackNormal(texture(texture3, uv).rgb);
	N =  getNormalFromMap(n, texture3, uv, fsin.position_M.xyz,dx,dy);
	vec3 L = normalize(l.direction);
	vec3 V = normalize(v);
...
void main(void) {
	...
	vec3 N =  getNormalFromMap(fsin.normal, texture3, texCoords, fsin.position_M.xyz,dx,dy);
