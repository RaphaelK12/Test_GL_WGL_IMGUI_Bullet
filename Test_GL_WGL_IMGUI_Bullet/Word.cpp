				...
				if (objetos[i]->malhas[0]->mMaterial)
					if (objetos[i]->malhas[0]->mMaterial->mData)
						if (objetos[i]->malhas[0]->mMaterial->mData->mShader) {
							objetos[i]->malhas[0]->mMaterial->mData->mShader->use();
...
void Word::refreshShaders() {
	for (uint i = 0; i < g_shader_list.size(); i++) {
		if (g_shader_list[i]->readSrcFromFilenames() > 1) {
			...
			g_shader_list[i]->use();
			g_shader_list[i]->setUniform("time", time);
			g_shader_list[i]->setUniform("frame", frame);
			g_shader_list[i]->setUniform("sintime", sintime);
			g_shader_list[i]->setUniform("costime", costime);
		}
	}
}
